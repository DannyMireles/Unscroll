...
extension LockStore {
    ...
    static func resolveAppStoreApp(named rawName: String) async -> AppStoreResolvedApp? {
        ...
        // The iTunes search gives us the real app artwork (the curated catalog doesn't have it).
        let searched = await searchAppStoreApp(term: term)

        if let known = resolveKnownCatalogApp(named: term) {
            NSLog(
                "🌐 Cuewell catalog resolved '%@' -> '%@' bundle=%@ scheme=%@",
                term,
                known.displayName,
                known.bundleID,
                known.launchSchemes.first ?? "nil"
            )
            // Prefer the curated identity/scheme, but borrow the App Store icon.
            return AppStoreResolvedApp(
                displayName: known.displayName,
                bundleID: known.bundleID,
                launchSchemes: known.launchSchemes,
                iconURLString: searched?.iconURLString
            )
        }
    ...
    private static func searchAppStoreApp(term: String) async -> AppStoreResolvedApp? {
        ...
        do {
            ...
            let icon = (best.artworkUrl512 ?? best.artworkUrl100)?
                .trimmingCharacters(in: .whitespacesAndNewlines)

            NSLog(
                "🌐 Cuewell App Store lookup '%@' -> '%@' bundle=%@ score=%d",
                term,
                best.trackName,
                best.bundleId,
            ...
            return AppStoreResolvedApp(
                ...
                iconURLString: (icon?.isEmpty == false) ? icon : nil
            )
        } catch {
            NSLog("🌐 Cuewell App Store lookup failed '%@': %@", term, String(describing: error))
            return nil
        }
    }
