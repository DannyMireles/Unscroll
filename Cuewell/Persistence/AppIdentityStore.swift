...
enum AppIdentityStore {
    private static let appGroup = "group.com.selerim.cuewell"
    ...
    private static let keychainAccessGroup = "GU6G649D6A.com.selerim.cuewell.keychain"
    private static let keychainService = "com.selerim.cuewell.appidentities"
    ...
    static func record(token: ApplicationToken, bundleID: String?, displayName: String?) {
        ...
        guard hasBundle || hasName else {
            NSLog("🧾 Cuewell identity: skipped record because bundle/name were both nil")
            return
        }
        guard let tokenData = encodedToken(token) else {
            NSLog("🧾 Cuewell identity: failed to encode token for '%@'", cleanName ?? cleanBundle ?? "unknown")
            return
        }
    ...
    static func logRoundTripCheck(for token: ApplicationToken, source: String) {
        ...
        NSLog(
            "🧾 Cuewell identity round-trip [%@] records=%d matches=%d bundle=%@ name=%@",
            source,
            records.count,
            matches.count,
    ...
    private static func save(_ records: [StoredIdentityRecord]) {
        guard let data = try? JSONEncoder().encode(records) else {
            NSLog("🧾 Cuewell identity: failed to encode %d record(s)", records.count)
            return
        }

        // Primary: shared keychain (securityd) — expected to work in the Shield sandbox.
        let keychainStatus = keychainSave(data)
        NSLog("🧾 Cuewell identity: keychain save status=%d for %d record(s)", Int(keychainStatus), records.count)
