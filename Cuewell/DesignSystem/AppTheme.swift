...
enum AppTheme {
    ...
    /// Accent tuned for icons/text that sit on glass/material chrome. In light mode it's the
    /// deep green; in dark mode it brightens to a legible mint — `accentDeep` is too dark to
    /// read against `.ultraThinMaterial` in dark mode.
    static let accentOnChrome = Color(uiColor: UIColor { traits in
        traits.userInterfaceStyle == .dark
            ? UIColor(red: 0.55, green: 0.86, blue: 0.72, alpha: 1)
            : UIColor(red: 0.11, green: 0.32, blue: 0.28, alpha: 1)
    })

    /// Hairline stroke for circular/material chrome controls that reads in both modes
    /// (the old fixed `white.opacity(0.4)` vanished on dark backgrounds).
    static let chromeStroke = Color(uiColor: UIColor { traits in
        traits.userInterfaceStyle == .dark
    ...
    static let tagline = "Train your mind. Unlock with intention."
    static let subtagline = "A quick mental cue before your apps."
...
extension View {
    ...
    @ViewBuilder
    func cuewellTypography() -> some View {
...
struct BrandAppLockMark: View {
    ...
    /// The locked app's real icon (from the App Store artwork) when we have it — shown small
    /// and bare with no background box. Falls back to a neutral glyph (never a letter
    /// placeholder) while the icon is loading or unavailable.
    @ViewBuilder
    private var appIcon: some View {
        if let urlString = lock.appIconURL, let url = URL(string: urlString) {
    ...
    private var appGlyph: some View {
        Image(systemName: lock.selectedWebDomainCount > 0 ? "globe" : "app.dashed")
