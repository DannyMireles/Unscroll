...
struct EditLockView: View {
    ...
    private var limitLabel: String {
        AppLock(selection: draft.selection, appDisplayName: "", dailyLimitMinutes: totalMinutes, unlockMethods: draft.unlockMethods).limitLabel
    }

    /// Bridges the lock's ordered method array to the multi-select grid's `Set` binding,
    /// writing back in the stable display order.
    private var methodsBinding: Binding<Set<UnlockMethod>> {
        Binding(
            get: { Set(draft.unlockMethods) },
            set: { newValue in
                draft.unlockMethods = UnlockMethod.allSelectable.filter { newValue.contains($0) }
            }
        )
    }
    ...
    private var methodSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionTitle(title: "Unlock Methods")
            UnlockMethodSelectionGrid(selection: methodsBinding)
        }
    }
    ...
    private func applyCapturedAppName(_ name: String, for token: ApplicationToken) {
        ...
        NSLog("🔗 Cuewell: captured edited app label '%@'", trimmed)
    }
    ...
    private func applyPickerCapturedAppNameIfPossible(_ name: String) {
        ...
        NSLog("🔗 Cuewell: captured edited picker app label '%@'", trimmed)
    }
