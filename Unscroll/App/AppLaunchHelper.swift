...
enum AppLaunchHelper {
    ...
    private static func openTargetApp(
        ...
        guard !candidates.isEmpty else {
            ...
            if shouldRetryIdentityResolution(for: lock, attempt: resolveAttempt) {
                ...
                NSLog("🔗 Unscroll: no launch candidates for '%@'; refreshing identity in %.1fs", displayName, delay)
                NotificationCenter.default.post(name: .unscrollRefreshIdentityReport, object: nil)
    ...
    private static func logLookupReadiness(for lock: AppLock) {
        ...
        NSLog(
            "🌐 Unscroll App Store lookup skipped: Screen Time provided token(s) only; no app name or bundle ID to search"
        )
    }
