...
struct ApplicationTokenNameCapture: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIViewController {
        ...
        NSLog("🔎 Unscroll token label capture mounted")
        return host
    }
                ...
                Self.recognizeText(in: image) { [weak self] names in
                    ...
                    for name in names {
                        if let sanitized = Self.sanitized(from: name) {
                            NSLog("🔎 Unscroll token OCR captured '%@'", sanitized)
                            self.emit(sanitized)
                            return
                        }
                    }
                    if delay == 0.7 || delay == 3.4 || delay == 12.0 {
                        NSLog("🔎 Unscroll token OCR empty at %.1fs raw=[%@]", delay, names.prefix(4).joined(separator: " | "))
                    }
                }
            } else if delay == 0.7 || delay == 3.4 || delay == 12.0 {
                let sample = found.prefix(4).joined(separator: " | ")
                NSLog("🔎 Unscroll token label capture empty at %.1fs raw=[%@]", delay, sample)
            }
        }
...
struct AppIdentityReportCapture: View {
        ...
        if #available(iOS 16.0, *),
           ...
           selection.webDomainTokens.isEmpty {
            DeviceActivityReport(reportContext, filter: reportFilter)
                ...
                .accessibilityHidden(true)
                .onAppear {
                    NSLog("🧾 Unscroll report capture mounted applications=%d", selection.applicationTokens.count)
                }
        }
    }
    ...
    private var reportContext: DeviceActivityReport.Context {
        DeviceActivityReport.Context("Unscroll App Identity Capture")
    }
    ...
    final class Coordinator {
        ...
        private func scan(delay: TimeInterval) {
            ...
            if let chosen = ranked.first?.name {
                ...
                NSLog(
                    "🧭 Unscroll picker label capture empty at %.1fs raw=[%@]",
                    delay,
        ...
        private func emit(_ name: String) {
            ...
            NSLog("🧭 Unscroll picker captured visible app label '%@'", name)
            onResolve(name)
        }
