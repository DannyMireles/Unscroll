...
final class ShieldActionExtension: ShieldActionDelegate {
    override func handle(
        ...
        case .primaryButtonPressed:
            ...
            scheduleOpenCuewellNotification(lockID: lockID, requestID: requestID) {
    ...
    private func scheduleOpenCuewellNotification(
        lockID: UUID?,
        requestID: String,
        afterScheduled: @escaping () -> Void
    ) {
        let identifier = "cuewell.open.activity.\(requestID)"

        let content = UNMutableNotificationContent()
        content.title = "Complete your activity"
        content.body = "Tap to open Cuewell."
        content.sound = .default

        let deeplink: String
        if let lockID {
            deeplink = "cuewell://unlock?id=\(lockID.uuidString)"
        } else {
            deeplink = "cuewell://unlock"
        }
        ...
        center.add(request) { error in
            if let error {
                NSLog("🔔 Cuewell shield action notification failed %@", String(describing: error))
            } else {
                NSLog("🔔 Cuewell shield action notification scheduled deeplink=%@", deeplink)
            }
