# Unscroll RevenueCat And Launch Handoff

This document has two jobs:

1. Give another Codex agent a complete implementation prompt for the Unscroll product, RevenueCat, paywall, and launch-readiness changes.
2. Define the company/product direction and the low-spend marketing plan for the first 30-60 days.

## Product Direction

Unscroll is the first paid product in a small portfolio of lightweight behavior-change apps. The company direction is to build calm, privacy-respecting tools that turn impulsive digital habits into intentional actions.

The first product is Unscroll:

- Mission: help people keep the apps they like without opening them automatically.
- Promise: Unscroll does not ban your apps. It adds a small mental speed bump before you scroll.
...
Future products can follow this pattern once Unscroll proves the loop:
...
Unscroll is the first focus because purchase intent is clearest and the app can launch without a backend.
...
```text
You are working in /Users/danielmireles/Desktop/SelerimProjects/Unscroll.

Goal:
Prepare Unscroll for a low-cost paid launch with RevenueCat, a coherent free/pro product model, a clean paywall, restore purchases, privacy-safe local behavior, and a shareable progress loop. Do not add a custom backend.

Important context:
- Unscroll is a SwiftUI iOS app using Apple's Screen Time APIs: FamilyControls, ManagedSettings, DeviceActivity, and Shield extensions.
...
- Products to configure in App Store Connect / RevenueCat:
  - unscroll_pro_monthly_499
  - unscroll_pro_annual_2999
  - unscroll_lifetime_7999
- Launch prices:
  - Monthly: $4.99
  - Annual: $29.99
  - Lifetime: $79.99
...
1. Add RevenueCat SDK
   ...
   - Add the package only to the main Unscroll app target, not the Screen Time extensions unless there is a proven need.
...
2. Add purchase state architecture
   - Create a small purchase layer, for example:
     - Unscroll/Purchases/PurchaseManager.swift
     - Unscroll/Purchases/PurchaseState.swift
     - Unscroll/Purchases/RevenueCatPurchaseManager.swift
     - Unscroll/Purchases/Entitlements.swift
   - Expose a main-actor ObservableObject with:
     ...
     - purchase(package:)
     - restorePurchases()
   - Check entitlement `pro` from CustomerInfo.
...
3. Wire purchase state into app launch
   - Instantiate the purchase manager in UnscrollApp.
...
11. App Store release readiness
   ...
   - Do not hardcode claims that the app blocks usage perfectly. Screen Time can be disabled by the device owner, so copy should frame Unscroll as intentional friction, not absolute enforcement.
...
1. Create a RevenueCat project named `Unscroll`.
2. Add the iOS app.
...
8. Attach the monthly, annual, and lifetime products to the offering.
...
Do not build a backend for Unscroll V1.
...
> Unscroll adds a quick mental challenge before the apps you open on autopilot, helping you keep the apps you like while breaking the reflex to scroll.
...
Use Looprail as the internal growth control plane for Unscroll. The goal is to build the content machine before paid spend.
...
5. Screen Time comparison
   - Explain that Apple Screen Time tells you what happened; Unscroll interrupts the reflex.
...
> So before I open it, Unscroll teaches me one Spanish word.  
...
> Unscroll adds a small challenge right there.  
